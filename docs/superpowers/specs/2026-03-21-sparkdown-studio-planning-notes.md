# Sparkdown Studio: Planning Notes

> Original planning document from the design session that produced the UI design spec.
> See also: `2026-03-21-sparkdown-studio-ui-design.md` for the finalized spec.
...
- Keyboard shortcut `Cmd+K` pins the whisper card → expands to a mini-editor where you can add/edit properties inline
...
Triggered by: `Cmd+Shift+R`, or clicking a document in the navigator when not in edit mode, or opening a shared/published view.
...
**View 2: Timeline**
For documents with temporal entities (Events with dates, Dublin Core dates), a horizontal timeline appears:
...
This is a UI design spec — implementation would involve creating a new `crates/sparkdown-studio/` crate (or a separate frontend repo) with:

1. **Editor component** — built on a performant text editor (e.g., wrapping a web-based editor like CodeMirror/Monaco, or a native editor like `iced` or `gpui`)
2. **Knowledge Panel** — renders entity details from `SemanticGraph` queries
3. **Constellation Bar** — graph visualization (D3.js force-directed or native equivalent)
4. **Mapping integration** — uses `MappingIndex::entities_at()` for cursor-position entity lookup
5. **Cross-document index** — a workspace-level index merging all sidecar files' entity registries
6. **Suggestion engine** — NLP-based entity detection feeding into the suggestion tray

### Key Sparkdown APIs to Leverage
- `MappingIndex::entities_at(range)` — cursor position → entities (O(log n))
- `SemanticGraph` — all entities, triples, and their statuses
- `Anchor::verify_snippet()` — staleness detection for inline indicators
- `SyncEngine::sync()` — post-edit anchor adjustment
- `sidecar::parse()` / `sidecar::serialize()` — read/write the semantic layer
- `ThemeRegistry` — ontology validation for entity type suggestions
- All three renderers for export functionality

### Verification
- Prototype the writing mode in a web-based editor to validate the ambient UX
- User-test the mode transition gradient to calibrate timing thresholds
- Benchmark `MappingIndex::entities_at()` with large documents (1000+ entities) to confirm real-time responsiveness
- Test cross-document entity unification with a corpus of 100+ notes
